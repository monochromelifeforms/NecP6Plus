...
The next step is to disassemble the ROM. The printer runs a V50 processor, which is an embedded x86-compatible processor.
Assuming the ROM is located at 0xE000 (so that it ends at 0xFFFF), we can start with the reset vector at 0xFFF0.
There is indeed a `jmp` instruction there. I have not proceeded beyond that point.
